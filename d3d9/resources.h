...
public:
	Resources(){
		...
		_text_padding_top = window_height*1.8f;
	...
	std::vector<TextAsset> get_text_assets(){
		RECT font_rect;
		SetRect(&font_rect, _text_center_position.x - _text_width / 2.0f, _text_center_position.y - _text_height / 2.0f, _text_center_position.x + _text_width / 2.0f, _text_center_position.x + _text_height/2.0f);
...
private:
	...
	// Text
	const float _text_width_percent = 2.0f;
	const float _text_height_percent = 2.0f;
	float _text_width;
	float _text_height;
	float _text_padding_top;
